...
function left_rg(ts::CTMTensors, chi)
    ...
    newC1 = renormalize(newC1)
    newE4 = renormalize(newE4)
    newC4 = renormalize(newC4)
...
function right_rg(ts::CTMTensors, chi)
    ...
    newC2 = renormalize(newC2)
    newE2 = renormalize(newE2)
    newC3 = renormalize(newC3)
...
function up_left(ts, C1::NestedTensor, E4::NestedTensor, C4::NestedTensor)
...
function up_right(ts, C2::NestedTensor, E2::NestedTensor, C3::NestedTensor)
    px = get(ts.Params, "px", 0.0f0)
    C2 = shift(C2, px)
    E2 = shift(E2, px)
    C3 = shift(C3, px)
...
function up_top(ts, C1::NestedTensor, E1::NestedTensor, C2::NestedTensor)
...
function up_bottom(ts, C4::NestedTensor, E3::NestedTensor, C3::NestedTensor)
    py = get(ts.Params, "py", 0.0f0)
    C4 = shift(C4, py)
    E3 = shift(E3, py)
    C3 = shift(C3, py)
