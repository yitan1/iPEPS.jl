...
function proj_left(ts, P, Pd)
    ...
    renormlize(newC1), renormalize(newE4), renormlize(newC4)
end
...
function proj_right(ts, P, Pd)
    ...
    renormlize(newC2), renormlize(newE2), renormlize(newC3)
end
...
function proj_top(ts, P, Pd)
    ...
    renormlize(newC1), renormlize(newE1), renormlize(newC2)
end
...
function proj_bottom(ts, P, Pd)
    ...
    newC4 = begin
        ...
        reshape(C1E4, size(C4E4,1), :)*P
    end

    newC3 = begin
        C3E2 = tcon([C3,E2],[[1,-1], [-4,1,-2,-3]])
        Pd*reshape(C3E2, :, size(C2E2, 4))
    end
    ...
    renormlize(newC4), renormlize(newE3), renormlize(newC3)
end
